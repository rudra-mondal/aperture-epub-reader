...
EpubReader.SPECIAL_CHARS_PRONUNCIATION = {' > ': ' is greater than ', ' < ': ' is less than ', '+': ' plus ', '=': ' equals ', ' - ': ' minus '}
def _optimized_pronounce_special_chars(self, text):
    for char, spoken in EpubReader.SPECIAL_CHARS_PRONUNCIATION.items():
        text = text.replace(char, spoken)
    return text
EpubReader._pronounce_special_chars = _optimized_pronounce_special_chars
...
def benchmark():
    ...
    print(f"Optimized Time taken for 100,000 iterations: {end_time - start_time:.6f} seconds")
